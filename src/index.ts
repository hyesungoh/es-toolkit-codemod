...
import { LODASH_FUNCTIONS_LOWER_CASE_TO_ORIGINAL_MAP } from './constants';
...
function transformLodashIndividualPackages(root: Collection, j: JSCodeshift): boolean {
  const lodashIndividualImports = root.find(j.ImportDeclaration).filter((path) => {
    ...
    return LODASH_FUNCTIONS_LOWER_CASE_TO_ORIGINAL_MAP.has(packageName);
  });
  ...
  lodashIndividualImports.replaceWith((path) => {
    ...
    const originalFunctionName = LODASH_FUNCTIONS_LOWER_CASE_TO_ORIGINAL_MAP.get(lowercaseFunctionName);
